/* tcl.h -- This header file describes the externally-visible facilities
...
#define TCL_MSDOS_PORT /* please comment define for other ports!! */
...
#define TCL_DYNAMIC ((Tcl_FreeProc *) xfree)
...
#  define ckalloc(x) xmalloc(x)
#  define ckfree(x)  xfree(x)
...
#define Tcl_FreeResult(interp)					\
    ...
    if ((interp)->freeProc == (Tcl_FreeProc *) xfree) {  \
        xfree((interp)->result);               \
	} else {						\
