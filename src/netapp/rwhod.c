...
PROCESS rwhod()
{
	...
	for( ; TRUE ; sleep(RWDELAY) ) {
		...
		len = RWMINP;
        if ( marked(Log.lmark) && Log.lused ) {
			rwwptr = &rpacptr->rw_rww[0];
			xinu_strcpy (rwwptr->rw_tty, "Console");
            strncpy(rwwptr->rw_nam, Log.luser, RWNLEN);
            rwwptr->rw_ton = hl2net(Log.llogin);
            rwwptr->rw_idle = hl2net(now - Log.llast);
        ...
        udpsend(nif[i].ni_brc, UP_RWHO, UP_RWHO, pep, len, 1);
	}
}
