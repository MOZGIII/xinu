...
u_char		pdutype;
{
	...
	*pp++ = ASN1_INT;
    ...
    for (cp = &rqdp->reqid[rqdp->reqidlen-1]; cp > rqdp->reqid; )/* was >= */
		*pp++ = *cp--;
            *pp++ = *cp--;          /* pointer fudge */
	*pp++ = rqdp->reqidlen;
	*pp++ = ASN1_INT;
	/* write the packet length and pdutype */
    len = a1writelen(tmpbuff, (int) (pp - snmpbuff));
    for (cp = &tmpbuff[len-1]; cp > tmpbuff; ) /* was >= */
		*pp++ = *cp--;
            *pp++ = *cp--;          /* pointer fudge */
	*pp++ = pdutype;
	...
	*pp++ = ASN1_SEQ;
    ...
    for (--pp, cp = snmppack; pp > snmpbuff; ) /* was >= */
		*cp++ = *pp--;
        *cp++ = *pp--;             /* pointer fudge */
	return cp - snmppack;
}
