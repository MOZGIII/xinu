    ...
    char **argv;			/* Argument strings. */
{
	...
	interp->result = (char*)getenv("TCL_LIBRARY");
    ...
    char **argv;			/* Argument strings. */
{
    ...
    interp->freeProc = (Tcl_FreeProc *) xfree;
    }
    if (parenthesized) {
    ...
    char **argv;			/* Argument strings. */
{
    ...
    if ((*argv[3] == 'e') && (strncmp(argv[3], "end", xinu_strlen(argv[3])) == 0)) {
    last = 10000;
    } else {
