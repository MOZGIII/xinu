    ...
    char **argv;			/* Argument strings. */
{
	...
	string = (char *) ckalloc((unsigned) (xinu_strlen(argPtr[1]) + 1));
...
#   define MAX_FIELDS 20
    ...
    arg1Length = (xinu_strlen(argv[1]) + 4) & ~03;
    ...
    char **argv;			/* Argument strings. */
{
	...
	if ((index >= 0) && (index < xinu_strlen(argv[2]))) {
	...
	sprintf(interp->result, "%d", xinu_strlen(argv[2]));
	return TCL_OK;
	...
	stringLength = xinu_strlen(argv[2]);
	if (Tcl_GetInt(interp, argv[3], &first) != TCL_OK) {
	    return TCL_ERROR;
	}
	if ((*argv[4] == 'e')
		&& (strncmp(argv[4], "end", xinu_strlen(argv[4])) == 0)) {
	    last = stringLength-1;
	} else {
	...
	if (right) {
	    ...
	    p = interp->result + xinu_strlen(interp->result) - 1;
