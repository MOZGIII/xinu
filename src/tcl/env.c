	...
	CONST char *value;		/* New value for variable. */
{
	...
	p = (char *) ckalloc((unsigned) (nameLength + xinu_strlen(value) + 2));
	environ[index] = p;
	strcpy(p, name);
    p += nameLength;
    *p = '=';
 ...
 */
 ...
 /* this function has been wiped out in the msdso port, because it is
...
EnvInit()
{
	...
	for (i = 0; i < length; i++) {
	newEnviron[i] = (char *) ckalloc((unsigned) (xinu_strlen(environ[i]) + 1));
