    ...
    char **argv;			/* Argument strings. */
{
	...
	if (argc == 4) {
	    if (strncmp(argv[3], "exec", xinu_strlen(argv[3])) != 0) {
	...
	MakeSpace(eventPtr, xinu_strlen(argv[2]) + 1);
	...
	iPtr->result = words;
    iPtr->freeProc = (Tcl_FreeProc *) xfree;
	return TCL_OK;
    }
    ...
    char *string;		/* String to substitute. */
{
    ...
    revPtr->newSize = xinu_strlen(string);
    ...
    char *string;		/* String to substitute. */
{
    ...
    revPtr->newSize = xinu_strlen(revPtr->newBytes);
    InsertRev(iPtr, revPtr);
}

/*
				 ...
				 * be modified. */
{
    ...
    eventPtr = &iPtr->events[iPtr->curEvent];
    size = xinu_strlen(eventPtr->command) + 1;
    ...
    if (&p[xinu_strlen(&eventPtr->command[bytesSeen]) + 1] >
	    &newCommand[size]) {
    ...
    char *new;			/* Replacement string for "old". */
{
    ...
    oldLength = xinu_strlen(old);
    newLength = xinu_strlen(new);
    src = cmd;
    count = 0;
    while (1) {
	src = strstr(src, old);
    ...
    length = xinu_strlen(cmd) + count*(newLength - oldLength);
				 ...
				 * a pattern. */
{
    ...
    result = (char *) ckalloc((unsigned) (xinu_strlen(command) + 1));
    dst = result;
