    ...
    char **argv;		/* Array of strings to concatenate. */
{
	...
	totalSize += xinu_strlen(argv[i]) + 1;
    }
	...
	*result = '\0';
	...
	for (length = xinu_strlen(element);
                             ...
                             * of a Tcl_FreeProc such as free. */
{
    ...
    if (oldFreeProc != 0) {
    if (oldFreeProc == (Tcl_FreeProc *) xfree) {
	    ckfree(oldResult);
	} else {
...

{
	...
	iPtr->appendUsed += xinu_strlen(string);
    }
    va_end(argList);
}

				 ...
				 * of new information may be added. */
{
	...
	iPtr->appendUsed = xinu_strlen(iPtr->result);
    }
    ...
    if (new == NULLPTR) {                               /* memory depleted */
    xinu_fprintf(STDERR,"TclSetupAppendBuffer: out of memory error\n");
    kill(getpid());                                     /* suicide */
    }   
    ...
    int *indexPtr;			/* Where to store index. */
{
    ...
    } else if (strncmp(string, "end", xinu_strlen(string)) == 0) {
	*indexPtr = 1<<14;
    } else {
